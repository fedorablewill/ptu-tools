...
function loadHomebrew(fileName) {
    $.get(`/json/${fileName}.dex.json`).done(function (result) {
        $("#homebrew-dex-json").val(JSON.stringify(result));
        buildToast("Applied Homebrew");
    }).fail(function () {
        alert(`Error loading file /json/${fileName}.dex.json`);
    });
    $.get(`/json/${fileName}.moves.json`).done(function (result) {
        $("#homebrew-moves-json").val(JSON.stringify(result));
    }).fail(function () {
        alert(`Error loading file /json/${fileName}.moves.json`);
    });
}
...
function searchSpecies(query) {
    ...
    if (homebrew) {
        try {
            // Check if valid JSON
            let homebrewDex = JSON.parse(homebrew);
            var homebrewData = [];

            homebrewDex.forEach(function (dexEntry) {
                var entryLabel = dexEntry["species"];
                if (dexEntry["form"]) {
                    entryLabel += ` (${dexEntry['form']})`;
                }
                homebrewData.push({
                    label: entryLabel,
                    value: "homebrew-" + entryLabel
                });
            });

            ajaxData["homebrewPokedex"] = homebrewData;
        } catch (e) {
    ...
    $.ajax("/speciesOptions", {
        ...
        alert("Error loading species options");
    });
    return species;
}